  ...
  const FilterPanel = (
    <div className="space-y-7">
        ...
        <div className="space-y-1.5">
          {MUNICIPALITIES.map((m) => (
            <label key={m} className="flex cursor-pointer items-center gap-2.5 rounded-xl px-2 py-1.5 text-sm text-ink transition hover:bg-slate-50">
              <input type="checkbox" checked={municipality === m} onChange={() => setMunicipality(municipality === m ? undefined : m)} className="h-4 w-4 accent-brand-500" />
              {m}
            </label>
          ))}
        </div>
      ...
      {municipality && barangays.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {barangays.map((b) => (
              <button key={b} onClick={() => setBarangay(barangay === b ? undefined : b)} className={cn('rounded-full border px-3 py-1.5 text-xs font-semibold transition', barangay === b ? 'border-brand-500 bg-brand-500 text-white' : 'border-slate-200 bg-white text-ink hover:border-brand-300')}>
                {b}
              </button>
            ))}
          </div>
      ...
      <div>
        ...
        <select value={school ?? ''} onChange={(e) => setSchool(e.target.value || undefined)} className="w-full cursor-pointer rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-navy-800 outline-none transition focus:border-brand-400">
      ...
      <div>
        ...
        <input type="range" min={1000} max={6000} step={100} value={maxRent ?? 6000} onChange={(e) => setMaxRent(Number(e.target.value) >= 6000 ? undefined : Number(e.target.value))} className="w-full accent-brand-500" />
        <div className="flex justify-between text-[11px] text-mut"><span>₱1,000</span><span>₱6,000+</span></div>
      </div>
      <div>
        <h4 className="mb-3 flex items-center gap-1.5 text-sm font-bold text-navy-800">
        ...
        <div className="flex flex-wrap gap-2">
          {ROOM_TYPES.map((t) => (
            <button key={t} onClick={() => toggleRoomType(t)} className={cn('rounded-full border px-3 py-1.5 text-xs font-semibold capitalize transition', roomTypes.includes(t) ? 'border-brand-500 bg-brand-500 text-white shadow-[0_6px_16px_rgb(30_115_232/0.3)]' : 'border-slate-200 bg-white text-ink hover:border-brand-300')}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-1.5">
          {GENDERS.map((g) => (
            <label key={g.value} className="flex cursor-pointer items-center gap-2.5 rounded-xl px-2 py-1.5 text-sm text-ink transition hover:bg-slate-50">
              <input type="radio" name="gender" checked={gender === g.value} onChange={() => setGender(gender === g.value ? undefined : g.value)} className="h-4 w-4 accent-brand-500" />
              {g.label}
            </label>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {AMENITY_KEYS.map((a) => (
            <button key={a.key} onClick={() => toggleAmenity(a.key)} className={cn('rounded-full border px-3 py-1.5 text-xs font-semibold transition', amenities[a.key] ? 'border-navy-800 bg-navy-800 text-white' : 'border-slate-200 bg-white text-ink hover:border-navy-300')}>
              {a.label}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {[4.5, 4.0, 3.5].map((r) => (
            <button key={r} onClick={() => setMinRating(minRating === r ? undefined : r)} className={cn('rounded-full border px-3 py-1.5 text-xs font-semibold transition', minRating === r ? 'border-brand-500 bg-brand-500 text-white' : 'border-slate-200 bg-white text-ink hover:border-brand-300')}>
              ★ {r}+
            </button>
          ))}
        </div>
      ...
      <label className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 transition hover:border-mint-300">
        ...
        <button onClick={() => setOnlyAvailable((v) => !v)} className={cn('relative h-6 w-11 rounded-full transition', onlyAvailable ? 'bg-mint-400' : 'bg-slate-200')} aria-label="Toggle available only">
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setDrawer(true)} className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-navy-800 transition hover:border-brand-300 lg:hidden">
            <SlidersHorizontal size={15} /> Filters {activeCount > 0 && <span className="flex h-5 w-5 items-center justify-center rounded-full bg-brand-500 text-[10px] text-white">{activeCount}</span>}
          </button>
          <label className="relative inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-navy-800 transition hover:border-brand-300">
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="cursor-pointer appearance-none bg-transparent pr-5 font-semibold outline-none">
        ...
        <div className="min-w-0 flex-1">
          {isLoading || isFetching ? (
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-[380px] rounded-[18px]" />)}
            </div>
          ) : data && data.length > 0 ? (
            <motion.div layout className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
              {data.map((h) => <HouseCard key={h.id} house={h} />)}
            </motion.div>
          ) : (
            <EmptyState icon={<MapPin size={22} />} title="No boarding houses match your filters" subtitle="Try widening your budget or clearing some filters." />
          )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {drawer && (
          <motion.div className="fixed inset-0 z-[80] bg-navy-950/50 backdrop-blur-sm lg:hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setDrawer(false)}>
            <motion.div className="absolute inset-y-0 left-0 w-[85%] max-w-sm overflow-y-auto bg-white p-6" initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', stiffness: 300, damping: 32 }} onClick={(e) => e.stopPropagation()}>
              <div className="mb-6 flex items-center justify-between">
                <h3 className="text-lg font-bold text-navy-800">Filters</h3>
                <button onClick={() => setDrawer(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-100"><X size={18} /></button>
              </div>
              {FilterPanel}
              <button onClick={() => setDrawer(false)} className="mt-6 w-full rounded-xl bg-brand-500 py-3 text-sm font-semibold text-white transition hover:bg-brand-600">
