...
CREATE TABLE `landlords` (
  ...
  `verified` tinyint(1) DEFAULT 0,
  `subscription` enum('none','basic','standard','premium') DEFAULT 'none',
...
-- location_pref/location_lat/location_lng are already declared in the
-- CREATE TABLE `landlords` above, and the API server adds them to older
-- databases on boot (ensureSchema). The multi-statement ALTER that used to live
-- here aborted the import with "Duplicate column name 'location_pref'" on every
-- fresh install, which stopped the rest of this file from running.
...
CREATE TABLE `users` (
  ...
  `role` enum('landlord','boarder','admin') NOT NULL,
  `name` varchar(255) NOT NULL,
...
(4, 'admin@gmail.com', 'admin123', 'admin', 'System Administrator', '0900 000 0000', '#0B2D63', NULL, '2026-09-14 00:00:00', '2026-09-14 00:00:00'),
